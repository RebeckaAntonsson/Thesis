...
# netMHC1 percentile score
def compute_netMHC1_percentile(netMHC1_df):
    
    netMHC1_percentile_score = (
        netMHC1_df.assign(immunogenic=netMHC1_df['netmhcpan_el percentile'] <= 1) # flags rows where percentile is below 1
    ...
    return netMHC1_percentile_score
...
# netMHC II percentile score
def compute_netMHC_II_percentile(netMHC_II_df):
    
    netMHC_II_percentile_score = (
        netMHC_II_df.assign(immunogenic=netMHC_II_df['netmhciipan_el percentile'] <= 10) # flags rows where percentile is below 10
            .groupby('antibody')['immunogenic'] # calculates mean of immunogenic for each antibody, gives the fraction
            .mean()
            .mul(100) # multiplies by 100 to get percentage
            .reset_index(name='netMHC_II_pep15_percentile')
        )
    return netMHC_II_percentile_score
